...
func GetMessagesHandler(c *gin.Context) {
	...
	if email == "" {
		logger.LogRequest(c, "get_messages", email, map[string]any{
			"status": "unauthorized",
		})
...
func GetMessageDetailHandler(c *gin.Context) {
	...
	email := c.GetString("email")
	if email == "" {
		logger.LogRequest(c, "get_message_detail", email, map[string]any{
			"status": "unauthorized",
		})
		c.JSON(http.StatusUnauthorized, gin.H{"error": "no identity"})
		return
...
func RandomizeHandler(c *gin.Context) {
	...
	if !ok {
		logger.LogRequest(c, "randomize_email", "", map[string]any{
			"status": "error",
			"error":  "cannot_generate_identity",
		})
	...
	logger.LogRequest(c, "randomize_email", email, map[string]any{
		"status": "ok",
	})
